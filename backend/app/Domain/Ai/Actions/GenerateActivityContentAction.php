...
/**
 * Génère un bloc `activity_contents`, ciblant une Activity OU un Sector —
 * jamais les deux, même XOR que la table elle-même (Phase 10). `$country`
 * à `null` produit un contenu générique (toutes juridictions) ; une valeur
 * produit un contenu spécifique à ce pays, qui prime toujours sur le
 * générique pour la même section (voir `ActivityContentBlocksQuery`).
 */
class GenerateActivityContentAction
{
    ...
    public function execute(Activity|Sector $subject, ?Country $country, ContentSection $section): AiGenerationJob
    {
        /** @var Collection<int, Fact> $facts */
        $facts = $subject->facts()->usable()->get();

        $prompt = AiPrompt::query()
            ->where('scope', ContentSectionScope::Activity)
        ...
        if ($prompt === null) {
            $this->finalizeJob->execute($job, GenerationStatus::Failed, errorMessage: 'Aucun prompt actif pour le scope activity.');
        ...
        if ($country !== null && ! $this->checkBudget->execute($country)) {
            $this->finalizeJob->execute($job, GenerationStatus::Rejected, errorMessage: 'Budget IA mensuel du pays dépassé.');

            return $job->fresh();
        }

        $userPrompt = $this->buildUserPrompt($prompt->user_template, $subject, $section, $facts);
        ...
        if ($country !== null) {
            $this->recordSpend->execute($country, $result->costCents ?? 0);
        }
        ...
        if (! $report->passed) {
            $this->writeContent($subject, $country, $section, (string) $result->rawOutput, ContentStatus::Review, $job, $facts);
            $this->finalizeJob->execute($job, GenerationStatus::NeedsReview, $result, implode(' ; ', $report->issues));

            return $job->fresh();
        }

        $this->writeContent($subject, $country, $section, (string) $result->rawOutput, ContentStatus::Generated, $job, $facts);
     ...
     */
    private function buildUserPrompt(string $template, Activity|Sector $subject, ContentSection $section, Collection $facts): string
    {
     ...
     * @param  Collection<int, Fact>  $facts
     */
    private function writeContent(Activity|Sector $subject, ?Country $country, ContentSection $section, string $body, ContentStatus $status, AiGenerationJob $job, Collection $facts): ActivityContent
    {
        $key = [
        ...
        $existing = ActivityContent::query()->where($key)->first();
        ...
        $content = ActivityContent::updateOrCreate(
            $key,
